...
public static class DataLoaderListBatchTests
{
    ...
    public static async Task Overflow_InternalBatch_Async()
    {
        ...
        var services = BuildServiceProviderWith<TestDataLoader>();
    ...
    public static async Task Ensure_Required_Keys_Are_Returned_One_Missing()
    {
        // arrange
        using var cts = new CancellationTokenSource(5000);
        var services = BuildServiceProviderWith<UnresolvedTestDataLoader>();
    ...
    private static ServiceProvider BuildServiceProviderWith<TDataLoader>()
        where TDataLoader : class, IDataLoader
    {
        return new ServiceCollection()
            .TryAddDataLoader2Core()
            .AddDataLoader<TDataLoader>()
    ...
    private sealed class TestDataLoader(
        ...
        : BatchDataLoader2<int, int[]>(batchScheduler, options)
    {
